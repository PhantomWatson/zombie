...
?>
function startWobblies(level, change_setting) {
	var html_tag = $$('html')[0];
	if (! html_tag.hasClassName('wobblies_enabled')) {
		html_tag.addClassName('wobblies_enabled');
	}
...
function stopWobblies() {
	var html_tag = $$('html')[0];
	if (html_tag.hasClassName('wobblies_enabled')) {
		html_tag.removeClassName('wobblies_enabled');
	}
...
function enableBlurries() {
	$('blurries_disable_button').show();
	$('blurries_enable_button').hide();
	var html_tag = $$('html')[0];
	if (! html_tag.hasClassName('blurries_enabled')) {
		html_tag.addClassName('blurries_enabled');
	}
	setGamestateValue('enable_blurries', 1);
}

function disableBlurries() {
	$('blurries_disable_button').hide();
	$('blurries_enable_button').show();
	var html_tag = $$('html')[0];
	if (html_tag.hasClassName('blurries_enabled')) {
		html_tag.removeClassName('blurries_enabled');
	}
	setGamestateValue('enable_blurries', 0);
}
	...
	new Ajax.Request(url, {
		...
		onSuccess: function(transport) {
			if (transport.responseText == '1') {
				...
				alert("Error. Whatever you were just trying to do didn't work. :(");
				return false;
			}
		}
	});
...
function showDrinkPanel() {
	if ($('talk_to_bartender')) {
		Effect.SlideUp('talk_to_bartender', {
			duration: 0.5
		});
	}
...
function showTalkPanel() {
	...
	Effect.SlideUp('talk_to_bartender', {
		duration: 0.5,
		afterFinish: function() {
			Effect.SlideDown('bartender_questions', {
				duration: 0.5
			});
		}
	});
...
function narrationNext() {
	var current_section = $$('section.sn_current')[0];
	var next_section = current_section.next('section');
	current_section.removeClassName('sn_current');
	current_section.addClassName('sn_prev');
	next_section.addClassName('sn_current');
}

function setupSplitNarration() {
	$$('section').each(function (this_block) {
		this_block.addClassName('split_narration');
		var next_block = this_block.next('section');
		if (next_block) {
			var next_button = new Element('input', {
				'type': 'button',
				'onclick': 'narrationNext()',
				'value': 'Next',
				'class': 'next'
			});
			var next_button_wrapper = new Element('div', {
				'class': 'next_wrapper'
			});
			next_button_wrapper.update(next_button);
			this_block.insert({'bottom': next_button_wrapper});
		}
	});
	$$('section')[0].addClassName('sn_current');
}